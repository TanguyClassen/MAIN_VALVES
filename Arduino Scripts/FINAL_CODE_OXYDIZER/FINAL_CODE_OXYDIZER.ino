...
int b_Homing_O = 0, w_Main_O = 0, b_SingleStep_O = 0;
int previous_homing_o = 0;
...
void loop() {
  ...
  if (client.available()) {
    ...
    if (!deserializeJson(doc, line)) {
      ...
      w_Main_O = doc["w_Main_O"].as<int>();
      b_SingleStep_O = doc["b_SingleStep_O"].as<int>();

      if (b_Homing_O == 1 && previous_homing_o == 0 && !is_calibrating) {
        runCalibration();
      }
      previous_homing_o = b_Homing_O;
      ...
      if (is_calibrated) {
        ...
        target_position = map(w_Main_O, 0, 100, pos_bottom, pos_top);
